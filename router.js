...
function dbConnect(dbId, handle, response, done) {
    ...
    if (connectionStatus.baseId == dbId && connectionStatus.status == true) {
        done(null);
    } else {
        console.log("Connect to database " + dbId + "..");

        if (config.db[dbId] && config.db[dbId].type == 'mysql') {
            connectionStatus.connection = mysql.createConnection({
            ...
            connectionStatus.connection.connect(function(err) {
                done(err);
            });
        }
            ...
            connectionStatus.connection = new pg.Client(conString);
